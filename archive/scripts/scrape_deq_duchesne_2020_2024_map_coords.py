...
DATA_FILE = os.path.join(BASE_DIR, "data", "deq_duchesne_2020_2024.json")
SEARCH_URL = "https://eqspillsps.deq.utah.gov/Search_Public.aspx"

SEARCH_DATE = "01/01/2020"
COUNTY_LABEL = "DUCHESNE"
...
def backup_file():
    backup = os.path.join(
        BASE_DIR,
        "data",
        f"deq_duchesne_2020_2024_before_coords_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
    )
...
def main():
    ...
    mapped = [
        r for r in records
        if r.get("the_geom") and r["the_geom"].get("coordinates")
    ]

    print()
    print("===================================")
    print("DEQ DUCHESNE COORDS DONE")
    print("===================================")
